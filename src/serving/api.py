...
from fastapi.middleware.cors import CORSMiddleware
from fastapi.middleware.gzip import GZipMiddleware
from fastapi.responses import JSONResponse  # ← ADDED for SageMaker endpoints
from contextlib import asynccontextmanager
import asyncio
import mlflow
import joblib
...
async def lifespan(app: FastAPI):
    ...
    # Connect to Redis (optional - continue even if Redis fails)
    try:
        cache = await redis.from_url("redis://localhost:6379", decode_responses=True)
        await cache.ping()
        logger.info("Redis connected")
    except:
        cache = None
...
# ── Existing prediction endpoints (unchanged) ──────────────────────────────────
...
# ── SageMaker REQUIRED endpoints (added) ──────────────────────────────────────

@app.get("/ping")
async def sagemaker_ping():
    """SageMaker health check — must return 200 for container to be marked healthy"""
...
async def sagemaker_invocations(request: Request):
    """SageMaker prediction endpoint — called by SageMaker runtime"""
    if circuit_breaker.is_open():
        raise HTTPException(503, "Service temporarily unavailable")
    try:
